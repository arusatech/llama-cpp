...
llama_cap_context_tts::llama_cap_context_tts(const std::string &vocoder_model_path, int batch_size) {
  ...
  model = init_result ? init_result->model() : nullptr;
  ctx = init_result ? init_result->context() : nullptr;
...
tts_type llama_cap_context_tts::getTTSType(llama_cap_context* main_ctx, json speaker) {
    ...
    {
        char desc[256] = {0};
        if (llama_model_desc(main_ctx->model, desc, sizeof(desc)) > 0) {
            if (std::string(desc).find("OuteTTS 0.1") != std::string::npos) {
                return OUTETTS_V0_1;
            }
        }
    }
